...
# Questo script JavaScript intercetta l'header della pagina Streamlit e forza
# il nome, i metadati e le icone per l'installazione su schermata Home (iOS e Android).
components.html(
    """
    <script>
        const docHead = window.parent.document.head;
        
        // Rimuove eventuali manifest o icone preesistenti di Streamlit se possibile
...
if st.session_state.active_tab == "📋 Visualizza Modelli":
    ...
    if selected_prod_name != "Tutti":
      ...
      def helper_filtra_pezzi(campo):
        ...
        if "motore" in c_low and "supporto" not in c_low:
          ...
          return [
              p
              for p in pezzi
              if p and p.get("Prodotto") and "corona" in p.get("Prodotto").lower()
          ]
          ...
          return [
              p
              for p in pezzi
              if p and p.get("Prodotto") and "telaio" in p.get("Prodotto").lower()
          ]
        elif "assale" in c_low:
          return [
              p
              for p in pezzi
              if p and p.get("Prodotto") and "assale" in p.get("Prodotto").lower()
          ]
        elif "cerch" in c_low:
          return [
              p
              for p in pezzi
              if p and p.get("Prodotto") and "cerch" in p.get("Prodotto").lower()
          ]
        elif "pickup" in c_low:
          return [
              p
              for p in pezzi
              if p and p.get("Prodotto") and "pickup" in p.get("Prodotto").lower()
          ]
      ...
      if selected_prod_name.lower() == "slot.it":
        ...
        if selected_prod_name.lower() == "nsr":
          ...
          idx_sosp_sc = (
              sosp_sc_opts.index(def_sosp_sc)
              if def_sosp_sc in sosp_sc_opts
              else 0
          )
          ...
          if sosp_scaleauto_attive == "Sì":
            ...
            def_molla_sc = (
                edit_data.get("Tipo_Molla_Scaleauto", "Molle Hard")
                if edit_data
                else "Molle Hard"
            )
            idx_molla_sc = (
                molla_sc_opts.index(def_molla_sc)
                if def_molla_sc in molla_sc_opts
                else 0
            )
          ...
          altre_tipologie = [
              ...
              and t
              not in ["Sospensioni", "Cuscinetti a flangia sin", "Bronzine"]
              and "distanzial" not in t.lower()
          ]
          tutte_le_principali = [
              p
              for p in priorita
              if any(x and x.get("Prodotto") == p for x in pezzi)
          ] + altre_tipologie
          ...
          for i, tipologia in enumerate(tutte_le_principali):
            ...
            with cols[i % 3]:
              scelte_utente[tipologia] = render_select_componente(
                  tipologia, sub_pezzi, "comp"
              )
      ...
      if selected_prod_name.lower() == "slot.it":
        ...
        with col_d_pick:
          dp_opts = ["No", "Sì"]
          def_dp = (
              edit_data.get("Distanziali_Pickup", "No") if edit_data else "No"
          )
          ...
          if dist_pick_attive == "Sì":
            ...
            scelte_utente["Distanziale_Pickup"] = render_select_componente(
                "Distanziale_Pickup", lista_dist_pick, "sel_dist_pick"
            )
      else:
        st.write("### 📏 Distanziali")
        ...
        with col_d1:
          da_opts = ["No", "Sì"]
          def_da = (
              edit_data.get("Distanziali_Anteriori", "No") if edit_data else "No"
          )
          ...
          if dist_ant_attive == "Sì":
            ...
            scelte_utente["Distanziale_Anteriore"] = render_select_componente(
                "Distanziale_Anteriore", lista_dist_ant, "sel_dist_ant"
            )

        with col_d2:
          dp_opts = ["No", "Sì"]
          def_dp = (
              edit_data.get("Distanziali_Posteriori", "No")
              if edit_data
              else "No"
          )
          idx_dp = dp_opts.index(def_dp) if def_dp in dp_opts else 0
          ...
          if dist_post_attive == "Sì":
            ...
            scelte_utente["Distanziale_Posteriore"] = render_select_componente(
                "Distanziale_Posteriore", lista_dist_post, "sel_dist_post"
            )
      ...
      if selected_prod_name.lower() in ["nsr", "scaleauto"]:
        ...
        lista_bronzine = [
            ...
            if p
            and p.get("Prodotto")
            and "bronz" in p.get("Prodotto").lower()
        ]
        scelte_utente["Dettaglio_Supporto"] = render_select_componente(
            "Dettaglio_Supporto", lista_bronzine, "sel_bronzine"
        )
      else:
        sup_opts = ["Bronzine", "Cuscinetti"]
        def_sup = (
            edit_data.get("Tipo_Supporto", "Bronzine")
            if edit_data
            else "Bronzine"
        )
        ...
        if scelta_tipo_supp == "Bronzine":
          ...
          lista_cuscinetti = [
              ...
              if p
              and p.get("Prodotto")
              and "cuscinett" in p.get("Prodotto").lower()
          ]
          scelte_utente["Dettaglio_Supporto"] = render_select_componente(
              "Dettaglio_Supporto", lista_cuscinetti, "sel_cuscinetti"
          )
  ...
  try:
    ...
    salvati = (
        response_garage.data if response_garage and response_garage.data else []
    )
  ...
  pulsante_edit = (
      st.session_state.modifying_pulsante_data
  ...
  if st.session_state.modifying_pulsante_id:
    st.info(
        "Stai modificando un pulsante esistente. Clicca su 'Aggiorna Impostazioni"
        " Pulsante' per confermare o annulla."
    )

  def_tipo_pulsante = (
      pulsante_edit.get("Tipo Pulsante", "Analogico")
      if pulsante_edit
      else "Analogico"
  )
  ...
  if scelta_tipo_pulsante == "Analogico":
    with col_bp2:
      ...
      def_mod_an = (
          pulsante_edit.get("Modello Specifico", "Savatteri")
          if pulsante_edit
          else "Savatteri"
      )
      idx_mod_an = (
          modelli_analogici_list.index(def_mod_an)
          if def_mod_an in modelli_analogici_list
          else 0
      )
    ...
    with col_bp2:
      ...
      def_mod_dig = (
          pulsante_edit.get("Modello Specifico", "Tic Tac")
          if pulsante_edit
          else "Tic Tac"
      )
      idx_mod_dig = (
          modelli_digitali_list.index(def_mod_dig)
          if def_mod_dig in modelli_digitali_list
          else 0
      )
  ...
  with col_bp3:
    ...
    def_pista = (
        pulsante_edit.get("Tipo Pista", "Ninco") if pulsante_edit else "Ninco"
    )
    idx_pista = piste_list.index(def_pista) if def_pista in piste_list else 0
    tipo_pista_val = st.selectbox("Tipo Pista", piste_list, index=idx_pista)

  tipo_auto_val = st.text_input(
      "Tipo Auto (Modello)",
      value=pulsante_edit.get("Tipo Auto", "") if pulsante_edit else "",
  )
  ...
  if scelta_tipo_pulsante == "Digitale" and scelta_modello_digitale == "Tic Tac":
    ...
    with col_tp1:
      ...
      def_mappa = (
          str(pulsante_edit.get("Mappa Potenza", "1"))
          if pulsante_edit
          else "1"
      )
      idx_mappa = (
          mappa_potenza_opts.index(def_mappa)
          if def_mappa in mappa_potenza_opts
          else 0
      )
      mappa_potenza_val = st.selectbox(
          "Mappa Potenza", mappa_potenza_opts, index=idx_mappa
      )

      potenza_val = st.text_input(
          "Potenza",
          value=pulsante_edit.get("Potenza", "") if pulsante_edit else "",
      )
      tipo_freno_val = st.text_input(
          "Tipo Freno",
          value=pulsante_edit.get("Tipo Freno", "") if pulsante_edit else "",
      )
    with col_tp2:
      valore_freno_val = st.text_input(
          "Valore Freno",
          value=pulsante_edit.get("Valore Freno", "") if pulsante_edit else "",
      )
      sensibilita_grilletto_val = st.text_input(
          "Sensibilità Grilletto",
          value=(
              pulsante_edit.get("Sensibilità Grilletto", "")
              if pulsante_edit
              else ""
          ),
      )
      antispin_val = st.text_input(
          "Antispin",
          value=pulsante_edit.get("Antispin", "") if pulsante_edit else "",
      )
      ...
      scelta_tipo_pulsante == "Digitale"
      and scelta_modello_digitale
      in ["SLOT.IT SCP-1", "SLOT.IT SCP-2", "SLOT.IT SCP-3"]
  ):
    col_tp1, col_tp2 = st.columns(2)
    with col_tp1:
      min_speed_val = st.text_input(
          "Min Speed",
          value=pulsante_edit.get("Min Speed", "") if pulsante_edit else "",
      )
      curve_val = st.text_input(
          "Curve",
          value=pulsante_edit.get("Curve", "") if pulsante_edit else "",
      )
      freno_val = st.text_input(
          "Freno",
          value=pulsante_edit.get("Freno", "") if pulsante_edit else "",
      )
    with col_tp2:
      power_trim_val = st.text_input(
          "Power Trim",
          value=pulsante_edit.get("Power Trim", "") if pulsante_edit else "",
      )
      mapping_opts = ["Slow", "Normal", "Fast"]
      def_mapping = (
          pulsante_edit.get("Mapping", "Normal") if pulsante_edit else "Normal"
      )
      idx_mapping = (
          mapping_opts.index(def_mapping) if def_mapping in mapping_opts else 1
      )
    ...
    col_tp1, col_tp2 = st.columns(2)
    with col_tp1:
      tuning_val = st.text_input(
          "Tuning",
          value=pulsante_edit.get("Tuning", "") if pulsante_edit else "",
      )
      sensibilita_val = st.text_input(
          "Sensibilità",
          value=pulsante_edit.get("Sensibilità", "") if pulsante_edit else "",
      )
    with col_tp2:
      freno_val = st.text_input(
          "Freno",
      ...
      th_min_val = st.text_input(
          "TH MIN",
          value=pulsante_edit.get("TH MIN", "") if pulsante_edit else "",
      )
      th_max_val = st.text_input(
          "TH MAX",
          value=pulsante_edit.get("TH MAX", "") if pulsante_edit else "",
      )
      ...
      scelta_tipo_pulsante == "Digitale"
      and scelta_modello_digitale == "SLOTING PLUS"
  ):
    col_tp1, col_tp2 = st.columns(2)
    with col_tp1:
      freno_val = st.text_input(
      ...
      start_val = st.text_input(
          "Start",
          value=pulsante_edit.get("Start", "") if pulsante_edit else "",
      )
    with col_tp2:
      antispin_val = st.text_input(
          "Antispin",
      ...
      def_curva_pot = (
          pulsante_edit.get("Curva di Potenza", "1") if pulsante_edit else "1"
      )
      idx_curva_pot = (
          curva_potenza_opts.index(def_curva_pot)
          if def_curva_pot in curva_potenza_opts
          else 0
      )
      curva_potenza_val = st.selectbox(
          "Curva di Potenza", curva_potenza_opts, index=idx_curva_pot
      )

      freno_fine_rettilineo_opts = ["On", "Off"]
      def_freno_rett = (
          pulsante_edit.get("Freno di fine Rettilineo", "Off")
          if pulsante_edit
          else "Off"
      )
      idx_freno_rett = (
          freno_fine_rettilineo_opts.index(def_freno_rett)
          if def_freno_rett in freno_fine_rettilineo_opts
          else 1
      )
      freno_fine_rettilineo_val = st.selectbox(
          "Freno di fine Rettilineo",
          freno_fine_rettilineo_opts,
          index=idx_freno_rett,
      )
    ...
    with col_tp1:
      ohm_val = st.text_input(
          "Ohm", value=pulsante_edit.get("Ohm", "") if pulsante_edit else ""
      )
      antispin_val = st.text_input(
          "Antispin",
          value=pulsante_edit.get("Antispin", "") if pulsante_edit else "",
    ...
    with col_tp2:
      ...
      def_spunto = (
          pulsante_edit.get("Tasto Spunto", "Su") if pulsante_edit else "Su"
      )
      idx_spunto = (
          tasto_spunto_opts.index(def_spunto)
          if def_spunto in tasto_spunto_opts
          else 0
      )
      tasto_spunto_val = st.selectbox(
          "Tasto Spunto", tasto_spunto_opts, index=idx_spunto
      )

      tasto_freno_opts = ["Tutto Freno", "Valori Potenziometro"]
      def_freno_tasto = (
          pulsante_edit.get("Tasto Freno", "Tutto Freno")
          if pulsante_edit
          else "Tutto Freno"
      )
      idx_freno_tasto = (
          tasto_freno_opts.index(def_freno_tasto)
          if def_freno_tasto in tasto_freno_opts
          else 0
      )
    ...
    with col_tp3:
      resistenza_val = st.text_input(
          "Resistenza",
          value=pulsante_edit.get("Resistenza", "") if pulsante_edit else "",
      )
      (
          min_speed_val,
          curve_val,
  ...
  if (
      st.session_state.modifying_pulsante_id
      and "modifying_pulsante_name" in st.session_state
  ):
      ...
      if st.button("💾 Aggiorna Impostazioni Pulsante"):
        if not nome_config_pulsante:
          ...
          try:
            ...
            st.success(
                f"Pulsante '{nome_config_pulsante}' aggiornato con successo!"
            )
            st.session_state.modifying_pulsante_id = None
            st.session_state.modifying_pulsante_data = None
            st.rerun()
  ...
  try:
    ...
    elenco_pulsanti = (
        response_pulsanti.data
        if response_pulsanti and response_pulsanti.data
        else []
    )
    ...
    if elenco_pulsanti:
      for pul in elenco_pulsanti:
        ...
        try:
          p_dict = (
              ast.literal_eval(p_dettagli_str)
              if isinstance(p_dettagli_str, str)
              else p_dettagli_str
          )
        except Exception:
