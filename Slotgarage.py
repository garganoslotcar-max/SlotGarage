...
# --- CONFIGURAZIONE STREAMLIT (DEVE ESSERE LA PRIMA COSA IN ASSOLUTO) ---
st.set_page_config(
...
def generate_pdf(config_name, modello_nome, dettagli, foto_url=None):
  ...
  for k, v in dettagli.items():
    if k.lower() == "note" or k.lower() == "foto_personalizzata_url": continue
    if k in ["Distanziali_Anteriori", "Distanziali_Posteriori"]: continue
    if k == "Distanziali_Pickup":
      ...
      else: dettagli_filtrati["Distanziale_Pickup"] = v
      continue
    if k == "Distanziale_Pickup": continue
...
if st.session_state.active_tab == "📋 Visualizza Modelli":
    ...
    if selected_prod_name != "Tutti":
      ...
      if selected_prod_name.lower() == "slot.it":
        ...
        with col1_slot:
          ...
          scelte_utente["Categoria_SlotIt"] = st.selectbox("Categoria", cat_slot_opts, index=idx_cat_slot, key=f"slotit_categoria_{model_safe_key}")
        with col2_slot:
        ...
        for idx, campo in enumerate(slotit_campi):
          with cols[idx % 3]:
            if campo == "Giri Motore":
              ...
              scelte_utente["Stopper"] = st.selectbox("Stopper", stopper_opts, index=idx_stop, key=f"slotit_stopper_{model_safe_key}")
            else:
        ...
        with col_tipo_sosp:
          ...
          tipo_sosp_slotit = st.selectbox("Tipo Sospensione", tipo_sosp_opts, index=idx_t_sosp, key=f"slotit_tipo_sospensione_{model_safe_key}")
        ...
        if selected_prod_name.lower() == "nsr":
          ...
          sosp_nsr_attive = st.selectbox("Sospensioni", sosp_nsr_opts, index=idx_sosp_nsr, key=f"nsr_sosp_sino_{model_safe_key}")
          ...
          if sosp_nsr_attive == "Sì":
            ...
            scelte_utente["Tipo_Molla_NSR"] = st.selectbox("Tipo Molla", molla_nsr_opts, index=idx_molla_nsr, key=f"nsr_tipo_molla_{model_safe_key}")
          ...
          for i, tipo_sosp in enumerate(tipi_sospensioni_thunderslot):
            ...
            with col_corrente:
              ...
              sosp_attive = st.selectbox("Stato", sosp_t_opts, index=idx_sosp_t, key=f"thunder_sosp_{key_base}_sino_{model_safe_key}")
              ...
              if sosp_attive == "Sì":
                ...
                tipo_materiale_sosp = st.selectbox("Tipo", tipo_m_opts, index=idx_tipo_m, key=f"thunder_sosp_{key_base}_tipo_{model_safe_key}")
                ...
                if tipo_materiale_sosp == "Molle":
                  ...
                  durezza_molla = st.selectbox("Durezza", dur_opts, index=idx_dur, key=f"thunder_sosp_{key_base}_durezza_{model_safe_key}")
          ...
          sosp_scaleauto_attive = st.selectbox("Sospensioni", sosp_sc_opts, index=idx_sosp_sc, key=f"scaleauto_sosp_sino_{model_safe_key}")
          ...
          if sosp_scaleauto_attive == "Sì":
            ...
            scelte_utente["Tipo_Molla_Scaleauto"] = st.selectbox("Tipo Molla", molla_sc_opts, index=idx_molla_sc, key=f"scaleauto_tipo_molla_{model_safe_key}")
        else:
          priorita = ["Motore", "Corona Sidewinder", "Pignone"]
          altre_tipologie = [t for t in list(set([p.get("Prodotto") for p in pezzi if p and p.get("Prodotto")])) if t not in priorita and t not in ["Sospensioni", "Cuscinetti a flangia sin", "Bronzine"] and "distanzial" not in t.lower()]
      ...
      if selected_prod_name.lower() == "slot.it":
        ...
        with col_d_pick:
          ...
          dist_pick_attive = st.selectbox("Distanziale Pickup", dp_opts, index=idx_dp, key=f"dist_pick_si_no_{model_safe_key}")
          scelte_utente["Distanziali_Pickup"] = dist_pick_attive
          if dist_pick_attive == "Sì":
            lista_dist_pick = [p for p in pezzi if p and p.get("Prodotto") and p.get("Prodotto").strip().lower() == "distanziale pickup"]
        ...
        with col_d1:
          ...
          dist_ant_attive = st.selectbox("Distanziali anteriori", da_opts, index=idx_da, key=f"dist_ant_si_no_{model_safe_key}")
          scelte_utente["Distanziali_Anteriori"] = dist_ant_attive
          if dist_ant_attive == "Sì":
            lista_dist_ant = [p for p in pezzi if p and p.get("Prodotto") and "distanzial" in p.get("Prodotto").lower() and "pickup" not in p.get("Prodotto").lower()]
        ...
        with col_d2:
          ...
          idx_dp = dp_opts.index(def_dp) if def_dp in dp_opts else 0
          dist_post_attive = st.selectbox("Distanziali posteriori", dp_opts, index=idx_dp, key=f"dist_post_si_no_{model_safe_key}")
          scelte_utente["Distanziali_Posteriori"] = dist_post_attive
          if dist_post_attive == "Sì":
            lista_dist_post = [p for p in pezzi if p and p.get("Prodotto") and "distanzial" in p.get("Prodotto").lower() and "pickup" not in p.get("Prodotto").lower()]
        ...
        with col_d3:
          ...
          dist_pick_attive = st.selectbox("Distanziale Pickup", dpk_opts, index=idx_dpk, key=f"dist_pick_si_no_{model_safe_key}")
          scelte_utente["Distanziali_Pickup"] = dist_pick_attive
          if dist_pick_attive == "Sì":
            lista_dist_pick = [p for p in pezzi if p and p.get("Prodotto") and p.get("Prodotto").strip().lower() == "distanziale pickup"]
            scelte_utente["Distanziale_Pickup"] = render_select_componente("Distanziale_Pickup", lista_dist_pick, "sel_dist_pick")
      ...
      if selected_prod_name.lower() in ["nsr", "scaleauto"]:
        ...
        lista_bronzine = [p for p in pezzi if p and p.get("Prodotto") and "bronz" in p.get("Prodotto").lower()]
        ...
        scelta_tipo_supp = st.selectbox("Seleziona componente", sup_opts, index=idx_sup, key=f"scelta_bronz_cusc_{model_safe_key}")
        scelte_utente["Tipo_Supporto"] = scelta_tipo_supp
        if scelta_tipo_supp == "Bronzine":
          lista_bronzine = [p for p in pezzi if p and p.get("Prodotto") and "bronz" in p.get("Prodotto").lower()]
          scelte_utente["Dettaglio_Supporto"] = render_select_componente("Dettaglio_Supporto", lista_bronzine, "sel_bronzine")
        else:
          lista_cuscinetti = [p for p in pezzi if p and p.get("Prodotto") and "cuscinett" in p.get("Prodotto").lower()]
  ...
  try:
    ...
    if salvati:
      for s in salvati:
        ...
        foto_auto_url = None
        if isinstance(dict_dettagli, dict) and dict_dettagli.get("foto_personalizzata_url"):
          foto_auto_url = dict_dettagli.get("foto_personalizzata_url")
        else:
          match_modello = next((m for m in modelli if m and m.get("name") == conf_modello), None)
        ...
        with col_btn_pdf:
          try:
            pdf_bytes = generate_pdf(conf_nome, conf_modello, (dict_dettagli if isinstance(dict_dettagli, dict) else {"Dettagli": dettagli_str}), foto_url=foto_auto_url)
            st.download_button(label="⬇️ PDF", data=pdf_bytes, file_name=f"{conf_nome.replace(' ', '_')}_scheda_tecnica.pdf", mime="application/pdf", key=f"download_pdf_{conf_id}")
          except Exception as e:
        ...
        with st.expander(f"Visualizza dettagli di {conf_nome}"):
          if foto_auto_url:
            try: st.image(foto_auto_url, width=200)
  ...
  st.write("Configura le impostazioni e i parametri tecnici del tuo pulsante di guida.")
  ...
  with col_bp1:
    scelta_tipo_pulsante = st.selectbox("Seleziona Tipo Pulsante", ["Analogico", "Digitale"], index=idx_tipo)
  ...
  if scelta_tipo_pulsante == "Analogico":
    with col_bp2:
      ...
      scelta_modello_analogico = st.selectbox("Seleziona Modello Analogico", modelli_analogici_list, index=idx_mod_an)
  elif scelta_tipo_pulsante == "Digitale":
    with col_bp2:
      modelli_digitali_list = ["Tic Tac", "C.O.S.A", "SLOT.IT SCP-1", "SLOT.IT SCP-2", "SLOT.IT SCP-3", "SLOTING PLUS"]
      ...
      scelta_modello_digitale = st.selectbox("Seleziona Modello Digitale", modelli_digitali_list, index=idx_mod_dig)
  ...
  if scelta_tipo_pulsante == "Digitale" and scelta_modello_digitale == "Tic Tac":
    ...
    with col_tp2:
      ...
      tasto_freno_val = st.selectbox("Tasto Freno", tasto_freno_opts, index=idx_freno_tasto)
    with col_tp3:
  ...
  nome_config_pulsante = st.text_input("Nome Configurazione Pulsante", value=default_nome_pulsante)
        ...
        if not nome_config_pulsante:
          ...
          try:
            dati_pulsante = {
                ...
                "Modello Specifico": (scelta_modello_analogico if scelta_tipo_pulsante == "Analogico" else scelta_modello_digitale),
            ...
            supabase.table("ilMioPulsante").update(record_pulsante).eq("id", st.session_state.modifying_pulsante_id).execute()
    ...
    if st.button("💾 Salva Impostazioni Pulsante"):
      if not nome_config_pulsante:
        ...
        try:
          ...
          st.success(f"Configurazione del pulsante '{nome_config_pulsante}' salvata con successo!")
          st.rerun()
        except Exception as e:
  ...
  with st.form("form_nuovo_modello"):
    ...
    carica_file_foto = st.file_uploader("Carica immagine dal dispositivo", type=["jpg", "jpeg", "png"])
    foto_modello_url = st.text_input("O inserisci URL Immagine Modello")

    prod_form_list = {p.get("name"): p.get("id") for p in produttori if p and p.get("name") and p.get("id")}
    cat_form_list = {c.get("name"): c.get("id") for c in categorie if c and c.get("name") and c.get("id")}
